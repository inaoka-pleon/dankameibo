...
class HaruhiganDetail extends Model
{
    use HasFactory;
    ...
    protected static function booted()
    {
        ...
        static::creating(function ($haruhiganDetail) {
            if (Auth::guard('web')->check()) {
                $loggedInUser = Auth::guard('web')->user();
                if (!empty($loggedInUser->jiin_id)) {
                    $haruhiganDetail->jiin_id = $loggedInUser->jiin_id;
                } else {
        ...
        static::updating(function ($haruhiganDetail) {
            if ($haruhiganDetail->isDirty('jiin_id')) {
                $haruhiganDetail->jiin_id = $haruhiganDetail->getOriginal('jiin_id');
            }
        });
    }
}