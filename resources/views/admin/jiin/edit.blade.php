<x-admin-layout>
    ...
    <main class="mt-1 py-2 px-2 sm:px-4">
        ...
        <div class="registration">
            <form action="{{ route('admin.jiin.update', $jiin->id) }}" method="post">
                @csrf
                @method('PATCH')
                ...
                <table class="registration-table">
                    <tbody>
                        <tr>
                            <input type="hidden" name="jiin_id" value="{{ $jiin->id }}"/>
                            <th class="registration-item">寺院名
                                <span class="registration-item-required">必須</span>
                            </th>
                            <td class="registration-body">
                                <input type="text" name="jiin_name" class="registration-input" placeholder="" value="{{ $jiin->jiin_name }}"/>
                            </td>
                        </tr>
                        <tr>
                            <th class="registration-item">備考</th>
                            <td class="registration-body">
                                <textarea type="text" name="memo" class="registration-textarea" value="{{ $jiin->memo }}"></textarea>
                            </td>
                        </tr>
                    </tbody>
                </table>
