    ...
    <main class="mt-1 py-4 px-2 sm:px-4">
        ...
        <div class="mb-4 flex flex-col justify-center items-center">
            <div class="flex justify-between bg-white border-l-8 border-b-2 border-slate-500 shadow-sm py-3 sm:py-4 px-4 sm:px-4 rounded-bl w-full max-w-7xl">
                <div class="text-gray-800 text-xl font-semibold">寺院情報 / 編集</div>
            </div>
        </div>
        ...
        <div class="registration">
            <form action="{{ route('member.update', $member->id) }}" method="post">
                ...
                <table class="registration-table">
                    <tbody>
                        <input type="hidden" name="temple_id" value="{{ $member->temple_id }}" />
                        <tr>
                            ...
                            <td class="registration-body">
                                <select id ="subtitle" name="subtitle" class="registration-select">
                                    ...
                                    @foreach($subtitles as $subtitle)
                                        <!-- @if(!is_null(old('subtitle')))
                                            //バリデーションエラー等による再表示時
                                            @if($subtitle->subtitle == old('subtitle'))
                            ...
                            <td class="registration-body">
                                <select id ="teacher" name="teacher" class="registration-select">
                                    ...
                                    @foreach($teachers as $teacher)
                                        <!-- @if(!is_null(old('teacher')))
                                            //バリデーションエラー等による再表示時
                                            @if($teacher->value1 == old('teacher'))
                                ...
                                <select id ="qualification" name="qualification" class="registration-select">
                                    ...
                                    @foreach($qualifications as $qualification)
                                        <!-- @if(!is_null(old('qualification')))
                                            //バリデーションエラー等による再表示時
                                            @if($qualification->value1 == old('qualification'))
                        ...
                        <tr>
                            ...
                            <td class="registration-body">
                                <textarea type="text" name="memo" class="registration-textarea" value="{{ $member->memo }}">{{ $member->memo }}</textarea>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="form-btn">
                    <a class="form-btn2 form-inline-block" href="{{ route('temple.show', $member->temple_id) }}">戻る</a>
