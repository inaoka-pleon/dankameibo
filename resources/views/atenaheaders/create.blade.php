    ...
    <main class="mt-1 py-4 px-2 sm:px-4">
        ...
        <div class="mb-4 flex flex-col justify-center items-center">
            <div class="flex justify-between bg-white border-l-8 border-b-2 border-slate-500 shadow-sm py-3 sm:py-4 px-4 sm:px-4 rounded-bl w-full max-w-7xl">
                <div class="text-gray-800 text-xl font-semibold">題名 / 新規作成</div>
            </div>
        </div>
        ...
        <div class="registration">
            <form action="{{ route('atenaheader.store') }}" method="post">
                @csrf
            ...
            <table class="registration-table">
                <tbody>
                    <tr>
                        <th class="registration-item">題名</th>
                        <td class="registration-body">
                            <input type="text" id="title" name="title" class="registration-input" placeholder="題名を入力してください" value="{{ old('title') }}" />
                        </td>
                    </tr>
                </tbody>
            </table>
            <div class="form-btn">
                <a class="form-btn2 form-inline-block" href="{{ route('atenaheader.index') }}">
