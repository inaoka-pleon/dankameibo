<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner">宗務所マスタ</h2>

        <!-- 宗務所新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('templeoffice.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white orverflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('templeoffice.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_templeoffice" class="text-xs">宗務所</label>
                                        <div>
                                            <input type="search" name="cond_templeoffice[templeoffice]" value="{{ $cond_templeoffice['templeoffice'] }}" class="text-xs w-full lg:w-1/2" id="k_templeoffice">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="templeoffice_search">指定条件で検索する</button>
                ...
                <!-- 宗務所一覧表示 -->
                @if($templeoffices->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="templeofficeno">番号</th>
                        ...
                        <tbody>
                            @foreach ($templeoffices as $templeoffice)
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            {{ $templeoffice->templeoffice }}
                                        </div>
                                    </td>
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('templeoffice.edit', $templeoffice->id) }}">編集</a>
                                        <form onsubmit="return deleteTempleoffice();"
                                            class="btn-delete"
                                            action="{{ route('templeoffice.destroy', $templeoffice->id) }}" method="post"
                    ...
                    <div class="pagination justify-content-center">
                        {{ $templeoffices->appends ([
                            'cond_templeoffice' => @(Request::get('cond_templeoffice')),
                            ])->links() }}
                    </div>
                @else
                    <div class="bu-blue-100 border-t border-b border-blue-500 texxt-blue-700 px-4 py-3" role="alert">
                        <p class="font-bold">宗務所の登録がありません</p>
                    </div>
                @endif
                </div> 
            </div>
