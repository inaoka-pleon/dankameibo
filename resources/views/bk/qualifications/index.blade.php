<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner">資格マスタ</h2>

        <!-- 資格新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('qualification.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white orverflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('qualification.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_qualification" class="text-xs">資格</label>
                                        <div>
                                            <input type="search" name="cond_qualification[qualification]" value="{{ $cond_qualification['qualification'] }}" class="text-xs w-full lg:w-1/2" id="k_qualification">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="qualification_search">指定条件で検索する</button>
                ...
                @if($qualifications->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="qualificationno">番号</th>
                        ...
                        <tbody>
                            @foreach ($qualifications as $qualification)
                                <tr>
                                    ...
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('qualification.edit', $qualification->id) }}">編集</a>
                                        <form onsubmit="return deleteQualification();"
                                            class="btn-delete"
                                            action="{{ route('qualification.destroy', $qualification->id) }}" method="post"
                    ...
                    <div class="pagination justify-content-center">
                        {{ $qualifications->appends ([
                            'cond_qualification' => @(Request::get('cond_qualification')),
                            ])->links() }}
                    </div>
                @else
                    <div class="bu-blue-100 border-t border-b border-blue-500 texxt-blue-700 px-4 py-3" role="alert">
                        <p class="font-bold">資格の登録がありません</p>
                    </div>
                @endif
            </div> 
        </div>
