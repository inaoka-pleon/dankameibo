<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner">檀家区分マスタ</h2>

        <!-- 檀家区分新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('dankadivision.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white orverflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('dankadivision.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_dankadivision" class="text-xs">檀家区分</label>
                                        <div>
                                            <input type="search" name="cond_dankadivision[dankadivision]" value="{{ $cond_dankadivision['dankadivision'] }}" class="text-xs w-full lg:w-1/2" id="k_dankadivision">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="dankadivision_search">指定条件で検索する</button>
                ...
                <!-- 檀家区分一覧表示 -->                
                @if($dankadivisions->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="code">番号</th>
                                <th class="dankadivision">檀家区分</th>
                                <th>
                                    <span></span>
                                </th>
                            </tr>
                        ...
                        <tbody>
                            @foreach ($dankadivisions as $dankadivision)
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            {{ $dankadivision->dankadivision }}
                                        </div>
                                    </td>
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('dankadivision.edit', $dankadivision->id) }}">編集</a>
                                        <form onsubmit="return deleteDankaDivision();"
                                            class="btn-delete"
                                            action="{{ route('dankadivision.destroy', $dankadivision->id) }}" method="post"
                    ...
                    <div class="pagination justify-content-center">
                        {{ $dankadivisions->appends ([
                            'cond_dankadivision' => @(Request::get('cond_dankadivision')),
                            ])->links() }}
                    </div>
                @else
                    <div class="bu-blue-100 border-t border-b border-blue-500 texxt-blue-700 px-4 py-3" role="alert">
                        <p class="font-bold">檀家区分の登録がありません</p>
                    </div>
                @endif
            </div> 
        </div>
