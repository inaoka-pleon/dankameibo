<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner">師マスタ</h2>

        <!-- 師新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('teacher.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white orverflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('teacher.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_teacher" class="text-xs">師</label>
                                        <div>
                                            <input type="search" name="cond_teacher[teacher]" value="{{ $cond_teacher['teacher'] }}" class="text-xs w-full lg:w-1/2" id="k_teacher">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="teacher_search">指定条件で検索する</button>
                ...
                @if($teachers->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="teacherno">番号</th>
                                <th class="teacher">師</th>
                                <th>
                                    <span></span>
                                </th>
                            </tr>
                        ...
                        <tbody>
                            @foreach ($teachers as $teacher)
                                <tr>
                                    ...
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('teacher.edit', $teacher->id) }}">編集</a>
                                        <form onsubmit="return deleteTeacher();"
                                            class="btn-delete"
