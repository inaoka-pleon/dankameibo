<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner">脇敬称マスタ</h2>

        <!-- 脇敬称新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('subtitle.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white orverflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('subtitle.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_subtitle" class="text-xs">脇敬称</label>
                                        <div>
                                            <input type="search" name="cond_subtitle[subtitle]" value="{{ $cond_subtitle['subtitle'] }}" class="text-xs w-full lg:w-1/2" id="k_subtitle">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="subtitle_search">指定条件で検索する</button>
                ...
                @if($subtitles->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="subtitleno">番号</th>
                                <th class="subtitle">脇敬称</th>
                                <th>
                                    <span></span>
                                </th>
                            </tr>
                        ...
                        <tbody>
                            @foreach ($subtitles as $subtitle)
                                <tr>
                                    ...
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('subtitle.edit', $subtitle->id) }}">編集</a>
                                        <form onsubmit="return deleteSubtitle();"
                                            class="btn-delete"
                                            action="{{ route('subtitle.destroy', $subtitle->id) }}" method="post"
