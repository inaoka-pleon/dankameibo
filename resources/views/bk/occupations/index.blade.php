<x-app-layout>
    <x-slot name="header">
        <h2 class="header header-inner header-nav header-nav-item">職業マスタ</h2>

        <!-- 職業新規登録画面に遷移 -->
        <div class="flex flex-row-reverse">
            <a class="header-btn" href="{{ route('occupation.create') }}">
                新規登録
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="hs-accordion-group">
                        <div class="hs-accordion active" id="hs-basic-heading-one">
                            ...
                            <div id="hs-basic-collapse-one" class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-heading-one">
                                <form aciton="{{ route('occupation.index') }}" method="GET" >

                                    <div class="my-1">
                                        <label for="k_occupation" class="text-xs">職業</label>
                                        <div>
                                            <input type="search" name="cond_occupation[occupation]" value="{{ $cond_occupation['occupation'] }}" class="text-xs w-full lg:w-1/2" id="k_occupation">
                                        </div>
                                    </div>
                                    <div class="my-4">
                                        <button type="submit" class="btn-primary proc-btn text-xs" name="searchType" value="occupation_search">指定条件で検索する</button>
                ...
                @if($occupations->isNotEmpty())
                    <table class="table_design01 radius-table">
                        <thead>
                            <tr>
                                <th class="occupationid">ID</th>
                                <th class="occupation">職業</th>
                                <th>
                                    <span></span>
                                </th>
                            </tr>
                        ...
                        <tbody>
                            @foreach ($occupations as $occupation)
                                <tr>
                                    ...
                                    <td class="btn-center">
                                        <a class="btn-edit" href="{{ route('occupation.edit', $occupation->id) }}">編集</a>
                                        <form onsubmit="return deleteOccupation();"
                                            class="btn-delete"
                                            action="{{ route('occupation.destroy', $occupation->id) }}" method="post"
